...
function LoginContent() {
  ...
  useEffect(() => {
    if (searchParams.get('logout') === 'success') {
      setSuccess("You've been signed out successfully. Please sign in again to continue.");
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="flex justify-between items-center mb-2">
              ...
              <Link
                ...
                className="text-xs text-[#BD7B54] hover:underline underline-offset-2 cursor-pointer"
              >
                Forgot password?
              </Link>
            </div>
            ...
            {success && (
              <div className="flex items-start gap-2.5 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3">
                <CheckCircle2 size={16} className="text-emerald-600 mt-0.5 flex-shrink-0" />
                <p className="flex-1 text-sm text-emerald-800">{success}</p>
                <button
                  type="button"
                  onClick={() => setSuccess('')}
                  className="text-emerald-600/50 hover:text-emerald-700 cursor-pointer flex-shrink-0"
                  aria-label="Dismiss"
                >
                  <X size={15} />
                </button>
          ...
          <p className="mt-8 text-sm text-[#20241F]/55">
            ...
            <Link
              href="/register"
              className="text-[#BD7B54] font-medium hover:underline underline-offset-2 cursor-pointer"
            >
              Create an account
            </Link>
          </p>
